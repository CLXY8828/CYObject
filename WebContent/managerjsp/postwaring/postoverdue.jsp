...
<html>
	...
	<%
		...
		<div class="container">
			...
			<!-- /*以下放数据 -->
			<c:forEach items="${list_time}" var="map" >
						<tr>
							<td><%=i++%></td>
						...
						<tr>
							<td colspan="9">
								<div class="row" style="background-color: #e5e5e5;">
									...
									<%}else{%>
										<a href="<%=request.getContextPath()%>/waring/over.do?method=query&page=<%=(pagej-1)%>"><span class="glyphicon glyphicon-chevron-left" style="width: 22px;height: 22px;border: #000000 1px solid;"></span></a>
									<%} %>
									...
									<%}else{%>
										<a href="<%=request.getContextPath()%>/waring/over.do?method=query&page=<%=(pagej+1)%>"><span class="glyphicon glyphicon-chevron-right" style="width: 22px;height: 22px;border: #000000 1px solid;"></span></a>
									<%} %>
									</div>
	...
	<script type="text/javascript">
		...
		function pagenum1(num) {
			if(num<=<%=(count/10)!=0?((count/10)+1):1%>){
				var url= "<%=request.getContextPath()%>/waring/over.do?method=query&page="+num+"";	
		...
		function checkdel(delform){
			if(confirm("确认删除该公司招聘信息")){
				var url= "<%=request.getContextPath()%>/waring/over.do?method=delete&eid="+delform.id+"&page="+page+"";
				window.location.href=encodeURI(url);
			}
		}
		function checkexport(){
			if(confirm("你将导出预警系统中未就业学生的信息")){
				var url= "<%=request.getContextPath()%>/waring/over.do?method=export";
				window.location.href=encodeURI(url);
			}else{
				return false;
			}
